// RailroadData.test.js
// Unit tests for the RailroadData component

// Testing imports
import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest';
import { mount } from '@vue/test-utils';
import { mockLineStrings } from './mock-data/mockGeoJSON';
...
// Component imports
import RailroadData from '@/components/RailroadData.vue';
...
describe('RailroadData.vue', () => {
	// Mock fetches to return mock data
	beforeEach(() => {
		global.fetch = vi.fn(() => {
			return Promise.resolve({
				ok: true,
				json: () => Promise.resolve(mockLineStrings),
			});
		});
	});
	...
	it('Populates element with paths', async () => {
		...
		const wrapper = mount(RailroadData, {
			props: {
				properties: {
					inputValue: ref(null),
